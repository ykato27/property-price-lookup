...
class AthomeScraper(BaseScraper):
    ...
    def build_search_url(
        ...
        athomeのURL構造:
        https://www.athome.co.jp/chuko/mansion/tokyo/
        """
        prefecture_slugs = {
            "東京都": "tokyo",
            "神奈川県": "kanagawa",
        ...
        # 中古マンション検索URL
        base_url = "https://www.athome.co.jp/chuko/mansion/"
    ...
    def parse_property_list(self, html: str) -> List[str]:
        ...
        property_cards = soup.select(
            ".property-list-item, .bukken-box, .property-box"
        )

        if not property_cards:
            property_cards = soup.find_all("div", class_=lambda x: x and ("bukken" in x.lower() or "property" in x.lower()))

        for card in property_cards:
            # 詳細リンクを抽出
            link = card.find("a", href=lambda x: x and ("/chuko/mansion/" in x or "/detail/" in x))
    ...
    def _extract_image_urls(self, soup: BeautifulSoup) -> List[str]:
        ...
        img_tags = soup.select(
            ...
            ".bukken-photo img, div[class*='photo'] img"
        )
        ...
        for img in img_tags:
            ...
            if src and self._is_valid_image_url(src):
                if src.startswith("//"):
                    ...
                    src = "https://www.athome.co.jp" + src
