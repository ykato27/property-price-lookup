...
class HomesScraper(BaseScraper):
    ...
    def build_search_url(
        ...
        pref_slug = prefecture_slugs.get(prefecture, "tokyo")

        # 中古マンション検索URL
        base_url = "https://www.homes.co.jp/mansion/chuko/"

        if page > 1:
            url = f"{base_url}{pref_slug}/list/?page={page}"
        else:
            url = f"{base_url}{pref_slug}/list/"
    ...
    def parse_property_list(self, html: str) -> List[str]:
        ...
        property_cards = soup.select(
            ".mod-bukkenList, .bukken-list, .property-list"
        )

        if not property_cards:
            property_cards = soup.find_all("li", class_=lambda x: x and "bukken" in x.lower())

        for card in property_cards:
            # 詳細リンクを抽出
            link = card.find("a", href=lambda x: x and ("/mansion/" in x or "/b-" in x))
    ...
    def parse_property_detail(self, html: str, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            price_elem = soup.select_one(
                ".priceLabel, .bukken-price, .price, "
                "span.priceLabel, div.priceLabel"
            )

            if not price_elem:
                price_dt = soup.find("dt", string=lambda x: x and "価格" in x)
                if price_dt:
                    price_elem = price_dt.find_next_sibling("dd")
            ...
            if not address_elem:
                address_dt = soup.find("dt", string=lambda x: x and ("所在地" in x or "住所" in x))
                if address_dt:
                    address_elem = address_dt.find_next_sibling("dd")
            ...
            for dl in dls:
                ...
                for dt, dd in zip(dts, dds):
                    ...
                    elif "交通" in label or "最寄駅" in label:
    ...
    def _extract_image_urls(self, soup: BeautifulSoup) -> List[str]:
        ...
        img_tags = soup.select(
            ...
            ".bukken-photo img, .mod-photo img, "
